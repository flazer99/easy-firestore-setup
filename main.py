...
def run_gcloud_script(deploy_id, project, region):
    """Runs the shell script and streams output to global storage."""
    
    cmd = ['./create_firestore.sh', project, region]
...
def get_regions():
    """Fetches list of available Google Cloud regions for Firestore."""
    try:
        ...
        # Firestore regions - these are the supported locations
        # Using compute regions as a proxy since Firestore uses similar regions
...
def deploy():
    ...
    thread = threading.Thread(
        ...
        args=(deploy_id, project_id, region)
    )
    thread.start()
